...
# UPLOAD FILES
# ==================================================
print("📤 Upload Transaction Input File")
input_file = list(files.upload().keys())[0]

print("\n📤 Upload System Client Master File")
system_client_file = list(files.upload().keys())[0]

print("\n📤 Upload System Scheme Master File")
...
# ==================================================
print("\n" + "="*60)
print("🔄 UPDATING CLIENT MASTER")
print("="*60)

# Read System Client Master
system_client = pd.read_excel(system_client_file)
print(f"\n📊 System Client Master loaded: {len(system_client)} rows")
...
print(f"\n📊 Master Client Master loaded: {len(master_client)} rows")
...
for target_col in target_columns:
    ...
    if normalized_target in system_client_normalized:
        ...
        print(f"   ✅ Matched: {target_col} -> {system_client_normalized[normalized_target]}")
    else:
        print(f"   ⚠️  Not found in System: {target_col}")
...
print(f"\n📊 Master Scheme Master loaded: {len(master_scheme)} rows")
...
for system_col, master_col in scheme_column_mapping.items():
    ...
    if normalized_system in system_scheme_normalized:
        ...
        print(f"   ✅ Matched: {system_col} -> {system_scheme_normalized[normalized_system]}")
    else:
        print(f"   ⚠️  Not found in System Scheme Master: {system_col}")
...
if len(missing_symbolids) > 0:
    ...
    print(f"\n✅ Updated Master Scheme Master: {len(master_scheme_updated)} rows")
else:
    master_scheme_updated = master_scheme.drop(columns=['_symbolid_clean'])
    print(f"\n✅ No new scheme records to add. Master is up to date!")

# ==================================================
# SAVE UPDATED MASTER FILE
# ==================================================
print(f"\n💾 Updating Master File with new Client Master and Scheme Master data...")
...
print(f"✅ Master File updated and saved as: {updated_master_file}")
...
print(f"   ✅ {matches.sum()} rows tagged as 'Ambit First'")
...
print(f"\n🔍 DEBUG: Transaction Descriptions NOT found in Master 'Trnx Type Update' sheet:")
if missing_values:
    ...
    print("   ✅ All transaction descriptions found in master!")
...
# ==================================================
print("\n🔹 Applying Ambit First overrides for Scheme Master columns...")
...
if ambit_override_count > 0:
    ...
    print(f"   ✅ {ambit_override_count} rows overridden with Ambit First values")
else:
    print(f"   ℹ️  No Ambit First rows to override")
...
print(f"📊 Client Master loaded: {len(client)} rows")
...
if duplicates_count > 0:
    print(f"⚠️  Found {duplicates_count} duplicate ClientCodes (keeping last occurrence)")

# Remove duplicates - keep LAST occurrence (often more recent)
client = client.drop_duplicates(subset=["_client_clean"], keep="last")

print(f"✅ After deduplication: {len(client)} unique client codes")
...
print(f"\n📋 Sample WS Codes from transaction file (first 5):")
for ws in df["_ws_upper"].unique()[:5]:
    print(f"   '{ws}'")

print(f"\n📋 Sample ClientCodes from master (first 5):")
...
# ==================================================
print("\n" + "="*60)
print("🔍 BANKER NAME LOOKUP DIAGNOSTICS")
print("="*60)
...
print(f"✅ Successful matches: {len(df) - missing_count} out of {len(df)} rows ({(len(df) - missing_count)/len(df)*100:.1f}%)")
print(f"❌ Missing Banker Name: {missing_count} out of {len(df)} rows ({missing_count/len(df)*100:.1f}%)")

if missing_count > 0:
    missing_ws = missing_banker["_ws_upper"].unique()
    print(f"\n📋 Unique WS Codes with missing Banker Name: {len(missing_ws)}")
    ...
    if in_master_but_failed:
        print(f"\n⚠️  WARNING: {len(in_master_but_failed)} WS codes exist in master but still failed to match!")
...
print("\n" + "="*60)
print("📊 WORKING SHEET COMPOSITION")
print("="*60)
